function [x, output] = CS_L1L2_uncon_FB_Adaptive_PR(A,b,pm)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfield(pm,'b_gt')
    b_gt  = pm.b_gt; 
    objtb = @(x) .5*norm(A*x-b_gt)^2 + lambda*(norm(x,1));
end
...
for it = 1:maxit
    % gradient
    f_new = (A'*(A*y-b));
    momentum_coef = min((f_new'*(f_new-f_old))/(f_old'*f_old), 1);
    f_old = f_new;
